...
def test_overlay_selects_only_recent_publications(tmp_path):
    ...
    conn.close()
    assert {r[0] for r in rows} == {"bger_7B_121/2026"}
    assert rows[0][2] == "recent ruling text"   # servable full text
    assert rows[0][3] == "2026-04-29"            # real (older) decision date preserved


def test_lookup_overlay_by_docket_and_by_id(tmp_path):
    inp = tmp_path / "bger.jsonl"
    _write_jsonl(inp, [
        {"decision_id": "bger_7B_121/2026", "docket_number": "7B_121/2026", "court": "bger",
         "decision_date": "2026-04-29", "publication_date": "2026-06-08",
    ...
    assert lookup_overlay(conn, "7B_121/2026")["decision_id"] == "bger_7B_121/2026"
    assert lookup_overlay(conn, "bger_7B_121/2026")["docket_number"] == "7B_121/2026"
...
def test_looks_like_bger_docket():
    assert mcp_server._looks_like_bger_docket("9C_104/2026")     # regular
    assert mcp_server._looks_like_bger_docket("I_594/2001")      # EVG single-letter
    assert mcp_server._looks_like_bger_docket("4P.226/2001")     # old dotted
    assert not mcp_server._looks_like_bger_docket("hello world")
    assert not mcp_server._looks_like_bger_docket("")
    assert not mcp_server._looks_like_bger_docket("bge_140 III 86")


def test_lookup_recent_overlay(tmp_path, monkeypatch):
    inp = tmp_path / "bger.jsonl"
    _write_jsonl(inp, [
        {"decision_id": "bger_7B_121/2026", "docket_number": "7B_121/2026", "court": "bger",
         "decision_date": "2026-04-29", "publication_date": "2026-06-08",
    ...
    r = mcp_server._lookup_recent_overlay("7B_121/2026")
    assert r and r["decision_id"] == "bger_7B_121/2026" and r["full_text"] == "fresh text"
    ...
    assert mcp_server._lookup_recent_overlay("7B_121/2026") is None
