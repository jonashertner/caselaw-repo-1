...
def test_fresh_manifest_emits_unique_keys(tmp_path):
    db = tmp_path / "decisions.db"
    _decisions_db(db, 100, 7)
    _manifest(tmp_path / "representation_manifest.db", {
        "algo_version": "2026-07-23.1",
        "source_user_version": "7",
        "estimated_unique_decisions": "92",
        "estimated_unique_lower_bound": "90",
        "duplicate_representations": "8",
    })
    out = gs._representation_dual_count(db, _conn(db))
...
def test_stale_manifest_withholds_unique_number(tmp_path):
    db = tmp_path / "decisions.db"
    _decisions_db(db, 100, 9)  # generation 9
    _manifest(tmp_path / "representation_manifest.db", {
        "algo_version": "2026-07-23.1",
        "source_user_version": "7",  # built against 7 -> stale
        "estimated_unique_decisions": "92",
        "estimated_unique_lower_bound": "90",
        "duplicate_representations": "8",
    })
...
def test_generate_stats_keeps_total_and_adds_dual_count(tmp_path):
    ...
    _manifest(tmp_path / "representation_manifest.db", {
        "algo_version": "v1", "source_user_version": "3",
        "estimated_unique_decisions": "45", "estimated_unique_lower_bound": "44",
        "duplicate_representations": "5",
    })
