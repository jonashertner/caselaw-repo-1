...
class WeblawBaseScraper(BaseScraper):
    ...
    def discover_new(self, since_date=None) -> Iterator[dict]:
        query_ticket = self._get_query_ticket()
        if not query_ticket:
            logger.error(f"[{self.court_code}] No query ticket")
            return

        url = f"{self.DOMAIN}{self.SUCH_URL}"
        body = f"s_word=&zips=&method=set_query&query_ticket={query_ticket}"
        ...
        while offset < total:
            body = (
                f"offset={offset}&s_pos=1&method=reload_query"
                f"&query_ticket={query_ticket}"
            )
            try:
                resp = self.post(url, data=body, headers=self.HEADERS)
                yield from self._parse_listing(resp.text, since_date)
                ...
                logger.warning(
                    f"[{self.court_code}] Page error at {offset}: {e}"
                )
                break
            offset += self.PAGE_SIZE
    ...
    def _get_query_ticket(self) -> str | None:
        try:
            ...
            match = _RE_QT.search(resp.text)
            return match.group("qt") if match else None
        except Exception as e:
            logger.error(f"[{self.court_code}] Form fetch failed: {e}")
            return None

    def _hit_count(self, html: str) -> int:
        match = _RE_TREFFER.search(html)
        return int(match.group("treffer")) if match else 0
    ...
    def _parse_listing(self, html: str, since_date=None) -> Iterator[dict]:
        ...
        for item in soup.select("ol li") or soup.select("div.result-item") or []:
            ...
            decision_id = make_decision_id(self.court_code, docket)
