...
class LocalClient:
    ...
    def __init__(self, pack: str | Path, *, log=None):
        ...
        self._con = sqlite3.connect(_sqlite_uri(self.pack_path), uri=True, check_same_thread=False)
        self._con.row_factory = sqlite3.Row
        try:
            self.meta = {r["key"]: r["value"] for r in self._con.execute("SELECT key, value FROM meta")}
        except sqlite3.DatabaseError as exc:
            raise APIError(200, f"{self.pack_path} is not a verification pack ({exc}); run `ocl pack pull`") from None
