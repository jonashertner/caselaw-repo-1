...
% practice, expanded Botschaft links). Wave-0 revision 2026-08-07 per the
% external major-revision review: telemetry demoted to operational lessons,
% records/collections terminology, PublG correction, unofficial-ECLI
% wording. The evaluation/audit content remains Paper 2 scope. All headline numbers regenerate via scripts/paper_snapshot_stats.py
...
% Frozen-snapshot figures used in the abstract. WAVE 1: swap these against
% the post-ATF/DTF freeze; the abstract prose itself carries no numbers.
\newcommand{\SnapDate}{2026-07-30}
\newcommand{\NRecords}{1{,}050{,}981}
\newcommand{\NExtracted}{9.66\,M}
\newcommand{\NResolved}{9.06\,M}
\newcommand{\NCoverage}{93.8\,\%}
\newcommand{\NLinks}{9.99\,M}
\newcommand{\NStatuteEdges}{12.42\,M}
\newcommand{\NProvisions}{298{,}922}

\title{OpenCaseLaw: An Open Multilingual Corpus and Citation Graph for Swiss Jurisprudence}
...
\begin{abstract}
OpenCaseLaw is an open multilingual corpus and citation graph for Swiss jurisprudence. The frozen \SnapDate{} release contains \NRecords{} decision records from 118 court and source collections across federal, cantonal, regulatory, and ECtHR layers in German, French, and Italian (1875--2026). A resolved citation graph links \NResolved{} of \NExtracted{} extracted citation tokens (\NCoverage{} coverage), represented as \NLinks{} source--target link rows, alongside \NStatuteEdges{} decision-to-statute edges over \NProvisions{} distinct provisions. Interpretive bridges connect provisions and decisions to Federal Council dispatches (\emph{Botschaften}) at article granularity, to open-licensed commentaries, to open-access scholarship with a bidirectional citation bridge, and to federal administrative practice. Every record carries a project-defined \texttt{cli:ch} identifier with an unofficial ECLI-compatible projection, a content hash, and a retrievable inclusion proof against a daily RFC~6962 Merkle root anchored via OpenTimestamps. We describe data acquisition, normalisation, citation and statute resolution, automated error diagnostics, licensing, coverage limits, and access through downloadable tables and REST/MCP interfaces under a citation-integrity serving contract. Project-created database packaging and metadata are dedicated to CC0; ECtHR-origin texts are distributed under their source terms; code is MIT.
\end{abstract}
...
Grounded legal retrieval is one downstream use with sharpened stakes: measurement studies report fabricated authority in 58--88\,\% of legal queries to general-purpose LLMs~\cite{dahl2024} and 17--33\,\% for commercial legal-RAG products~\cite{magesh2024}, and a public registry documents over 1{,}300 court proceedings in which judges confronted AI-generated citations~\cite{charlotin2026}. Checking a citation requires an open substrate to check it against; OpenCaseLaw is designed to provide that substrate for Swiss law. The resource itself, not this use case, is the contribution.
...
\item A deployed, agent-consumable serving layer: 42 MCP tools under a citation-integrity contract, REST with an OpenAPI subset, daily Parquet export, and a per-decision inclusion-proof API.
\end{enumerate}
...
\paragraph{Agent-accessible legal infrastructure.} The Model Context Protocol~\citep{mcpspec2025,mcpsurvey2025} is a widely used integration protocol between LLM agents and external tools. Section~\ref{sec:serving} describes the MCP interface and its citation-integrity serving contract.
...
\midrule
Caselaw Access Project~\cite{cap2018} & US      & no  & static CSV & no  & no  & yes (Free Law Project)  \\
...
\paragraph{Layered identifier (cli:ch + ECLI-compatible projection).} Every decision record carries a project-defined canonical identifier \texttt{cli:ch} with an unofficial ECLI-compatible projection following the Council-of-EU ECLI syntax~\citep{ecli2011}, both surfaced in the per-decision Schema.org/LegalCase JSON-LD. Switzerland has not designated a national ECLI coordinator; the projection implies no institutional adoption. cli:ch makes three features first-class that an ECLI-only scheme flattens: (i)~\emph{language as an axis}. A Federal Supreme Court judgment is rendered in one official language, normally the language of the contested decision (Art.~54 BGG); the official BGE report adds regesta translated into the other official languages, and several sources publish translated or parallel variants of headnotes and reports. The corpus therefore holds language-labelled records of differing character --- original judgments, translated regesta, publication variants --- and \texttt{?lang=} addresses them without implying equal legal authenticity. (ii)~\emph{cantonal + chamber granularity} (\texttt{cli:ch:zh:obergericht:...} preserves the 26 cantonal systems and their chambers rather than collapsing to an opaque court code); (iii)~\emph{pinpoint addressing} (Erwägung-level fragments like \texttt{\#e-2.3}, matching the Schweizer-Citation convention). An optional \texttt{@YYYY-MM-DD} suffix addresses a specific anonymisation version when needed. The compact form is
\begin{center}
\texttt{cli:ch:<court>[:<chamber>]:<docket>[@<version>][?lang=<l>][\#<pinpoint>]}.
\end{center}
The mapping to the ECLI syntax is a pure function (\texttt{cli\_ch\_to\_ecli}); the projection is many-to-one (lossy on language, pinpoint, version, chamber detail), so it does not satisfy ECLI's one-identifier-per-decision expectation and is offered as a crosswalk, not a registry. An HTTP resolver at \url{https://opencaselaw.ch/cli/ch/} plus the court/chamber/docket path redirects to the canonical decision page; browsers preserve fragment pinpoints across the 302. Identifier hygiene is enforced at the serving layer: legacy identifier forms containing spaces ($\sim$5.6\,\% of stored ids) are percent-encoded at every URL construction site, with a source-scan regression test pinning the invariant after an external report showed truncated links producing non-unique identifiers.

\paragraph{Cryptographic provenance (RFC-6962 + OpenTimestamps).} Every daily publish computes an RFC-6962~\citep{rfc6962} Merkle tree over all decision records in the corpus, anchored via OpenTimestamps~\citep{opentimestamps}. Each leaf commits to the tuple \texttt{(decision\_id, cli:ch, ECLI, content\_hash, decision\_date)}. We commit the 32-byte SHA-256 root (hash-domain-separated per the Certificate-Transparency RFC) to the public Git repository at \path{docs/integrity/<YYYY-MM-DD>.root} and anchor it on the Bitcoin blockchain via OpenTimestamps (\path{.root.ots}). The root for the present snapshot, over 1{,}050{,}981 decision records on 2026-07-30, begins \texttt{f5fc6229} (full value in the repository's integrity directory). A \texttt{GET /api/integrity/<decision\_id>} endpoint returns an inclusion proof in $O(\log n) \approx 20$ sibling hashes, computed in milliseconds against a memoised subtree cache. Any client can independently verify that a given decision was in the corpus on a given date without trusting the server: leaf $\to$ inclusion-proof walk $\to$ root $\to$ OpenTimestamps proof $\to$ Bitcoin block. Inclusion verification thus becomes a public API surface, not an internal release-engineering detail.
...
%==================================================================
\section{Agent-native serving layer}\label{sec:serving}

Legal-research consumers include LLM agents, and a static dump alone does not expose serving-time constraints or coverage notices. OpenCaseLaw is therefore served natively over the Model Context Protocol~\citep{mcpspec2025}: \textbf{42 public MCP tools} (search across every layer; per-Erwägung pinpoint retrieval; citation-graph walks; statute, Materialien, commentary, scholarship and practice lookups; batch retrieval; a deep-research shim) alongside REST with an OpenAPI subset and a daily CC0 Parquet export.
...
\paragraph{Operational lessons.} Operating the endpoint exposed access patterns a static release would not: bulk corpus export dominating raw request volume, episodic high-volume agent campaigns, templated monitors issuing standardised query streams, and whole documents pasted as search queries. These observations motivated serving-layer changes including input-length caps with instructive error messages, query condensation for pasted documents, in-band coverage disclosures, and per-tool functional monitoring. We do not report usage volumes as adoption evidence: the available counters mix transport opens with requests and identify neither people nor deduplicated sessions, and single consumers can dominate any aggregate.
...
\paragraph{Reproducibility capsule.} Corpus and graph counts derive from the frozen snapshot at \path{docs/paper/p1-resource/tables/corpus_graph_stats.json}, regenerated from the serving databases by the committed \path{scripts/paper_snapshot_stats.py}; \path{docs/paper/p1-resource/scripts/build_tables.py} regenerates the tables. Operational figures --- tool count, test count, audit-check count, smoke cadence --- are repository and deployment facts as of the snapshot date, verifiable from the cited scripts rather than from the frozen JSON. The automated error diagnostics live at \path{benchmarks/citation_precision_proxies.json}; the rule-consistency 400-row audit sample (frozen 2026-05-21, schema v1 field \texttt{adjudication}, read as \texttt{rule\_consistent} under schema v2) at \path{benchmarks/citation_precision_sample_400.jsonl}. The per-tool functional check is \path{scripts/tool_surface_check.py}. The 2026-07-30 integrity root sits in \path{docs/integrity/2026-07-30.root} and begins \texttt{f5fc6229}. We mint the archival DOI and submission git tag at submission, after this manuscript and the release artifacts are frozen.
