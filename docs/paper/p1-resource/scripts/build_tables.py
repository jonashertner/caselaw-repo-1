"""Regenerate p1-resource paper tables from corpus_graph_stats.json.

The JSON is produced by scripts/paper_snapshot_stats.py against the serving
databases (schema 2026-07); this builder consumes that schema directly."""
import json
...
if PROXY_PATH.exists():
    ...
    for mt in ["docket_norm", "bge_bare", "bge_norm", "bge_pincite", "oracle_xref"]:
    ...
    write(
        ...
        f"""\\begin{{table}}[H]
\\centering
\\caption{{Citation-resolution automated error diagnostics computed over the {fmt_int(overall['total'])} link-edge rows of the released graph (matching $E_{{\\textrm{{link}}}}$, Section~\\ref{{sec:graph}}; re-run on the {SNAPSHOT_DATE} frozen snapshot). Date-sanity pass = \\% of pairs where target decision date is not after the source's; violations are recorded-date chronology inconsistencies and indicate likely false-positive resolutions (conditional on the recorded source/target dates themselves being correct; we preserve source-portal-provided dates, see Limitations), not a lower bound on precision. Self-links are excluded by construction (resolver joins under target $\\neq$ source). Conf.\\,p50 = median resolver confidence per stratum. A rule-based mechanical consistency check over the shipped 400-row audit set is reported below; full human semantic adjudication remains companion-paper scope.}}
...
m = {(r["source_lang"], r["target_lang"]): r["n"] for r in s["cross_lang_matrix"]}
...
mat_docs = mat.get("botschaft_documents", 0)
mat_paras = mat.get("botschaft_paragraphs", 0)
mat_links = mat.get("article_botschaft_links", 0)
...
for r in s["top30_cited"][:15]:
    ...
    rows.append(f"\\texttt{{{tid_tex}}} & {reporter} & {fmt_int(r['n'])} \\\\")
