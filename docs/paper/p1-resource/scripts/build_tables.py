...
if PROXY_PATH.exists():
    ...
    write(
        ...
        f"""\\begin{{table}}[H]
\\centering
\\caption{{Citation-resolution precision proxies computed over the {fmt_int(overall['total'])} link-edge rows of the released graph (matching $E_{{\\textrm{{link}}}}$, Section~\\ref{{sec:graph}}; re-run on the {SNAPSHOT_DATE} frozen snapshot). Date-sanity pass = \\% of pairs where target decision date is not after the source's; violations are recorded-date chronology inconsistencies and indicate likely false-positive resolutions (conditional on the recorded source/target dates themselves being correct; we preserve source-portal-provided dates, see Limitations), not a lower bound on precision. Self-cite = pairs where source = target. Conf.\\,p50 = median resolver confidence per stratum. A rule-based mechanical consistency check over the shipped 400-row audit set is reported below; full human semantic adjudication remains companion-paper scope.}}
